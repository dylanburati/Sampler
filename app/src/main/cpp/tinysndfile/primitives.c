        ...
        const void *src, uint32_t src_mask, size_t sample_size, size_t count)
{
    ...
    if (dst_mask == src_mask) {
        memcpy(dst, src, sample_size * __builtin_popcountl(dst_mask) * count);
        return;
    }
    switch (sample_size) {
    case 1: {
        ...
        uint32_t dst_mask, uint32_t src_mask)
{
    ...
    return n + __builtin_popcountl(ormask & dst_mask);
}

size_t memcpy_by_index_array_initialization_src_index(int8_t *idxary, size_t idxcount,
        uint32_t dst_mask, uint32_t src_mask) {
    size_t dst_count = __builtin_popcountl(dst_mask);
    if (idxcount == 0) {
        return dst_count;
    }
